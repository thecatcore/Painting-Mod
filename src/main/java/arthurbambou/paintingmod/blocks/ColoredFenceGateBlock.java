...
public class ColoredFenceGateBlock extends FenceGateBlock {
    public ColoredFenceGateBlock(String name, Settings block$Settings_1, String modID) {
        ...
        if (modID == PaintingMod.MODID) {
            Registry.ITEM.register(new Identifier(modID, name), new BlockItem(this, new Item.Settings().itemGroup(PaintingMod.MOD_FENCE_GATES)));
        } else {
            Registry.ITEM.register(new Identifier(modID, name), new BlockItem(this, new Item.Settings().itemGroup(PaintingMod.ADDON_FENCE_GATES)));
        }
    }
}
