...
for (var i = 0; blocks.length > i ; i++) {
    ...
    for (var a = 0; colors.length > a ; a++) {
        ...
        var blockstate = {
            multipart: [
                { apply: { model: "paintingmod:" + colors[a] + "_" + blocks[i] + "_wall_post"}},
                { when: { north: true},
                    apply: { model: "paintingmod:" + colors[a] + "_" + blocks[i] + "_wall_side", uvlock: false}
                },
                { when: { east: true},
                    apply: { model: "paintingmod:" + colors[a] + "_" + blocks[i] + "_wall_side", y: 90, uvlock: false}
                },
                { when: { south: true},
                    apply: { model: "paintingmod:" + colors[a] + "_" + blocks[i] + "_wall_side", y: 180, uvlock: false}
                },
                { when: { west: true},
                    apply: { model: "paintingmod:" + colors[a] + "_" + blocks[i] + "_wall_side", y: 270, uvlock: false}
                }
            ]
        }
